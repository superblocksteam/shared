...
import { FieldListFormItem, FormComponentType, FormItem, InputFormItem, Plugin, Property } from '../types';
...
export function maskSecrets(datasourceConfig: DatasourceConfiguration, plugin: Plugin): void {
  plugin?.datasourceTemplate?.sections.forEach((section) => {
    section.items.forEach((section) => {
      if ((section as InputFormItem).dataType === InputDataType.PASSWORD) {
        maskPassword(datasourceConfig, section);
      } else if (section.componentType === FormComponentType.FIELD_LIST) {
        maskFieldList(datasourceConfig, section);
      }
    });
  });
}
